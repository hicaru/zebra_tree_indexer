...
description: Languages and chunking strategies supported by zebraindex
---

# Supported Languages

zebraindex uses **tree-sitter** to parse source code into an AST, then extracts semantic chunks
...
For files without a tree-sitter frontend (config, docs, generated code), zebraindex falls back
...
zebraindex language frontends live in `crates/zti-ts-*`. Each crate wraps a tree-sitter grammar
and implements the `Frontend` trait from `zti-tree-sitter`. Adding a new language means:
